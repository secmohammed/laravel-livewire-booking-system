<div class="bg-gray-200 max-w-sm mx-auto m-6 p-5 rounded-lg">
    <form action="" wire:submit.prevent="createBooking">
        <div class="mb-6">
            <label for="service" class="inline-block text-gray-700 font-bold mb-2">Select Service</label>
            <select name="service" id="service" class="bg-white h-10 w-full border-none roundded-lg" wire:model="state.service">
        ...
        <div class="mb-6 {{ !$employees->count() ? 'opacity-25': '' }}">
            <label for="employee" class="inline-block text-gray-700 font-bold mb-2">Select Service</label>
            <select name="employee" id="employee" class="bg-white h-10 w-full border-none roundded-lg" wire:model="state.employee" {{ !$employees->count() ? "disabled": ''}}>
                <option value="" >Select an employee</option>
                @foreach($employees as $employee)
                <option value="{{ $employee->id }}">{{ $employee->name }} </option>
                @endforeach
            </select>
            @error('state.employee')
            <div class="font-semibold text-red-500 text-sm mt-2">
                {{ $message }}
            </div>
            @enderror
        ...
        <div class="mb-6 {{ !$this->selectedService || !$this->selectedEmployee ? 'opacity-25': '' }}">
            <label for="service" class="inline-block text-gray-700 font-bold mb-2">Select Appointment time</label>
            ...
            <div class="mb-6">
                ...
                <div class="border-t border-b bordder-gray-300 py-2">
            ...
            <div class="mb-6">
                <div class="mb-3">
                    ...
                    <input type="text" id="name" name="name" class="bg-white h-10 w-full border-none roundded-lg" wire:model.defer="state.name">
                    @error('state.name')
                    <div class="font-semibold text-red-500 text-sm mt-2">
                        {{ $message }}
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="email" id="email" name="email" class="bg-white h-10 w-full border-none roundded-lg" wire:model.defer="state.email">
                    @error('state.email')
                    <div class="font-semibold text-red-500 text-sm mt-2">
                        {{ $message }}
                    </div>
            ...
            <button type="submit" class="bg-indigo-500 text-white h-11 px-4 text-center font-bold roundded-lg w-full"> Book now</button>
            @endif
        </form>
    </div>
